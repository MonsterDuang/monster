<template>
  <v-touch class="Song" @pan="onPan">
    <div class="play">
      <span class="btn-img">
        <i-circle :percent="percent" :stroke-width='2' :trail-width='2' stroke-color='#281f72' stroke-linecap="round" style="width:100%;height:110%">
...
export default {
  created () {
    if (this.nowPlayList !== 3) {
      this.$store.dispatch('getEusSong')
        .then(res => {
          this.$store.dispatch('getEusSongUrl')
        })
    }
  },
  computed: {
...
.Song{
  ...
  height: 100%;
  padding-top: 10%;
  background: rgba(45, 32, 142, 0.1);
  color: rgba(45, 32, 142, 0.5);
}
.play{
  width: 100%;
  height: 40%;
