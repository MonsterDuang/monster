<template>
  <v-touch class="Song" @pan="onPan">
    <div class="play">
      <span class="btn-img">
        <i-circle :percent="percent" :stroke-width='2' :trail-width='2' stroke-color='#188a3f' stroke-linecap="round" style="width:100%;height:110%">
...
export default {
  created () {
    if (this.nowPlayList !== 2) {
      this.$store.dispatch('getHotSong')
        .then(res => {
          this.$store.dispatch('getHotSongUrl')
        })
    }
  },
  computed: {
...
.Song{
  ...
  height: 100%;
  padding-top: 10%;
  background: rgba(23, 173, 75, 0.1);
  color: rgba(23, 173, 75, 0.5);
}
.play{
  width: 100%;
  height: 40%;
